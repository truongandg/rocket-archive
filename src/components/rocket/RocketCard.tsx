...
function successRate(rocket: Rocket) {
  if (!rocket.total_launch_count || rocket.successful_launches === null) return "—";
...
export default function RocketCard({ rocket }: RocketCardProps) {
  ...
  const manufacturer = rocket.manufacturer?.abbreviation || rocket.manufacturer?.name || "UNKNOWN MANUFACTURER";
  ...
  return (
    <article className="group relative overflow-hidden border border-white/15 bg-[#090909] transition-colors hover:border-white/50">
      <Link to={`/rockets/${rocket.id}`} className="block" aria-label={`View ${rocket.full_name || rocket.name}`}>
        ...
        <div className="px-5 pb-5 pt-5">
          <p className="text-[10px] font-semibold tracking-[.15em] text-white/45">{manufacturer.toUpperCase()}</p>
          <h3 className="mt-2 text-xl font-semibold tracking-[-.03em]">{rocket.name.toUpperCase()}</h3>
          <p className="mt-1 min-h-4 text-xs text-white/45">{family ? `${family.toUpperCase()} FAMILY` : rocket.full_name}</p>
          <div className="mt-6 grid grid-cols-3 border-t border-white/15 pt-4 text-[10px] font-semibold tracking-[.1em]">
            <div><span className="block text-white/40">LAUNCHES</span><span className="mt-1 block">{rocket.total_launch_count ?? "—"}</span></div>
            <div><span className="block text-white/40">SUCCESS</span><span className="mt-1 block">{rocket.successful_launches ?? "—"}</span></div>
            <div><span className="block text-white/40">RATE</span><span className="mt-1 block">{successRate(rocket)}</span></div>
          </div>
        </div>
      </Link>
